...
function getComparator<Key extends string | keyof CommuneData>( // Permitir strings para propiedades anidadas
	order: Order,
	orderBy: Key
): (a: Record<string, any>, b: CommuneData) => number {
...
export default function CustomTable({
	...
	const handleFilterSelection = () => {
		...
		isFilterSelection ? setData(dataCommunes) : setData(dataSelected);
	};
						...
						<Grid item xs={6} display={"flex"} justifyContent={"flex-end"}>
							{selected.length > 0 && (
								<CustomButton
									...
									children={
										isFilterSelection ? "Mostrar todo" : "Ver selección"
									}
